...
def get_part_data_by_name(self, name):
    if not hasattr(self, "parts_list"):
        ...
        self.parts_list = [
            ...
            {"name": "9873138580-LT", "description": "8580-LT"},
            {"name": "9846995180-LT", "description": "5180-LT"},
            {"name": "9873138780-LT", "description": "8780-LT"},
        ]
...
def init_camera():
    ...
    if global_camera is None or not global_camera.isOpened():
        ...
        # Try MJPG for USB cameras
        global_camera.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*"MJPG"))
        # Resolution
        global_camera.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)
        global_camera.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)
        # ENABLE AUTO SETTINGS
        global_camera.set(cv2.CAP_PROP_AUTOFOCUS, 1)
        # IMPORTANT
        global_camera.set(cv2.CAP_PROP_AUTO_EXPOSURE, 3)
        # Auto white balance
        global_camera.set(cv2.CAP_PROP_AUTO_WB, 1)
        # Remove manual brightness/contrast first
        # global_camera.set(cv2.CAP_PROP_BRIGHTNESS, 150)
        # global_camera.set(cv2.CAP_PROP_CONTRAST, 32)
        time.sleep(2)
...
class InspectorPage(tk.Frame):
    ...
    def run_inspection(self):
        ...
        if not ret:
            self.controller.after(
                0, lambda: messagebox.showerror("Error", "Camera capture failed.")
            )
            return
        active_part = self.controller.active_part  # or correct variable name
        part_data = self.controller.get_part_data_by_name(active_part)
        if not part_data:
            self.controller.after(
                0,
                lambda: messagebox.showerror(
                    "Error", f"ROIs or metadata for part '{active_part}' not found."
                ),
            )
            return
        if not part_data:
            self.controller.after(
                0,
                lambda: messagebox.showerror(
                    "Error", f"ROIs or metadata for part '{identified_part}' not found."
                ),
            )
            return
        part = part_data["name"]
        ...
        if not os.path.exists(paths["reference"]) or not os.path.exists(paths["roi"]):
            self.controller.after(
                0,
                lambda: messagebox.showerror(
                    "Error", "Reference image or ROIs missing."
                ),
            )
            return
        try:
            ...
            self.controller.after(
                0, lambda: messagebox.showerror("Error", f"Failed to load ROIs: {e}")
            )
            return
        ...
        for entry in rois:
            ...
            try:
                # Resize ROI
                ROI_SIZE = (150, 150)
                ref_roi = cv2.resize(ref_roi, ROI_SIZE)
                live_roi = cv2.resize(live_roi, ROI_SIZE)
                # Mild blur
                ref_roi = cv2.GaussianBlur(ref_roi, (3, 3), 0)
                live_roi = cv2.GaussianBlur(live_roi, (3, 3), 0)
                # Stable normalization
                ref_roi = cv2.normalize(ref_roi, None, 0, 255, cv2.NORM_MINMAX)
                live_roi = cv2.normalize(live_roi, None, 0, 255, cv2.NORM_MINMAX)
                # SSIM
                score = ssim(ref_roi, live_roi, data_range=255)
                # Stable rounding
        ...
        resized = cv2.resize(result, (1024, 768))
...
"""
        ...
        if not all_ok:
            GPIO.output(NG_PIN, GPIO.HIGH)  # NG ON
            time.sleep(10)  # wait 10 seconds
            GPIO.output(NG_PIN, GPIO.LOW)  # NG OFF
        else:
            GPIO.output(NG_PIN, GPIO.LOW)
"""
...
class AdminPage(tk.Frame):
    ...
    def on_mouse_down(self, event):
        ...
        self.current_rect = self.canvas.create_rectangle(
            self.start_x, self.start_y, event.x, event.y, outline="yellow", width=2
        )
...
class LoginPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        try:
            ...
            parts_data = [
                {"name": "9873138380-LT", "description": "Default part description"}
            ]
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.frames = {}
        self.active_part = "9873138380-LT"
