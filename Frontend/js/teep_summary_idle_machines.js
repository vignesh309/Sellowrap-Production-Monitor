...
async function loadTEEPReport() {
    ...
    tbody.innerHTML = `<tr><td colspan="17" style="text-align: center; color: var(--text-muted); padding: 30px;">Fetching Asset Analytics...</td></tr>`;
    ...
    try {
        ...
        // 🚨 Pointing to the NEW Idle Machines endpoint
        const response = await fetch(`/api/get_teep_summary_idle_machines?${params.toString()}`);
        ...
        if (!records || records.length === 0) {
            tbody.innerHTML = `<tr><td colspan="17" style="text-align: center; color: var(--status-yellow); padding: 30px;">No records found for this criteria.</td></tr>`;
            return;
        }
        ...
        tbody.innerHTML = `<tr><td colspan="17" style="text-align: center; color: var(--status-red); padding: 30px;">Error loading data. Check console.</td></tr>`;
    }
}